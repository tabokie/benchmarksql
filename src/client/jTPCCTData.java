...
public class jTPCCTData
{
    ...
    private boolean				terminalPlMode = false;
    ...
    public void setPlMode(boolean plMode) {
        terminalPlMode = plMode;
    }

    public boolean getPlMode() {
        return terminalPlMode;
    }
    ...
    throws Exception
    {
        ...
        switch (transType)
        {
            ...
            case TT_DELIVERY_BG:
            if (terminalPlMode) {
                executeDeliveryBGWithPl(log, db);
            } else {
    ...
    private void executeNewOrderWithPl(Logger log, jTPCCConnection db)
    throws Exception {
        ...
        try {
            ...
            */
            ...
            for (int i = 0; i < ol_cnt; i++) {
                ...
                ol_supply_w_id[i] = newOrder.ol_supply_w_id[seq];
                ol_quantity[i] = newOrder.ol_quantity[seq];
            }
        ...
        {
            log.error("Unexpected SQLException in NewOrderWithPl");
    ...
    * *********************************************************************/
    ...
    private void executePaymentWithPl(Logger log, jTPCCConnection db)
    throws Exception {
        PreparedStatement stmt;
		ResultSet               rs;
		...
		{
			log.error("Unexpected SQLException in PaymentWithPl");
			for (SQLException x = se; x != null; x = x.getNextException())
			log.error(x.getMessage());
			se.printStackTrace();
    ...
    private void executeDeliveryBGWithPl(Logger log, jTPCCConnection db)
    throws Exception {
        PreparedStatement stmt;
		try {
			stmt = db.stmtDeliveryBGPl;
			stmt.setInt(1, deliveryBG.w_id);
			stmt.setInt(2, deliveryBG.o_carrier_id);
			stmt.execute();
		} catch (SQLException se)
		{
			log.error("Unexpected SQLException in DeliveryBGWithPl");
			for (SQLException x = se; x != null; x = x.getNextException())
			    log.error(x.getMessage());
			se.printStackTrace();
